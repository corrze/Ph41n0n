...
# FIXED VERSION - Start training from scratch with proper piece rotations
# COMPATIBLE WITH PVP MODE
...
# Base piece shapes (starting orientations)
BASE_PIECES = {
...
# Changed this just in case we need to normalize rotations
def unique_rotations(shape: np.ndarray) -> List[np.ndarray]:
    """Generate unique rotations for a piece shape"""
    rots, seen = [], set()
    cur = shape
    for _ in range(4):
        # Normalize the shape to ensure consistent comparison
        # Convert to tuple of tuples for reliable hashing
        normalized = tuple(tuple(row) for row in cur)
...
# Verify piece rotations on import
print("🔍 VERIFYING FIXED PIECE ROTATIONS:")
